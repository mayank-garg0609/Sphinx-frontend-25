...
const GlowGlitchLogo: React.FC<GlowGlitchLogoProps> = ({
  ...
  const animateParticles = () => {
    ...
    if (Math.random() < 0.1) {
      setParticles(prev => [...prev, ...generateParticles().slice(0, 2)]);
    }
  ...
  return (
    ...
    >
      <div className={`${styles.glowEffect} ${isAnimating ? styles.animate : ''}`} />
      
      <div className={`${styles.scanlines} ${isAnimating ? styles.animate : ''}`} />
      
      <div className={styles.electricLines}>
        {[...Array(5)].map((_, i) => (
          <div
            key={i}
            className={`${styles.electricLine} ${isAnimating ? styles.animate : ''}`}
            style={{
              left: `${20 + i * 15}%`,
