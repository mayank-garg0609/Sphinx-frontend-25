...
const TeamHeader: React.FC = () => {
  ...
  return (
    <header className="fixed top-0 left-0 right-0 z-50 bg-[#0a0a0a]/95 backdrop-blur-md border-b border-white/5">
      <div
        className={`max-w-full mx-auto flex items-center justify-between ${styles.container}`}
      >
        <div className={`flex items-center ${isMobile ? "gap-3" : "gap-4"}`}>
          <div
            className={`${styles.logo} rounded-full overflow-hidden flex items-center justify-center`}
          >
            <Image
              src={logo}
              alt="Sphinx 25 Logo"
              width={
                isExtraLarge ? 80 : isLargeDesktop ? 64 : isMobile ? 48 : 64
              }
              height={
                isExtraLarge ? 80 : isLargeDesktop ? 64 : isMobile ? 48 : 64
              }
